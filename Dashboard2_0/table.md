...
Таблиця відтворюватиметься зі стовпцями `colA`, `colB` і `colC`, якщо «Columns» не визначено явно на вузлі, а «Автоматичні стовпці» вимкнено. Також можна надіслати окремий фрагмент даних для додавання до існуючої таблиці, у цьому випадку `ui-table` очікує введення (`msg.payload`) у форматі:
...
![image-20260104194722292](meida/image-20260104194722292.png)
...
рис.2. Вигляд робочого столу прикладу таблиці інтерфейсу користувача

![Mobile View of the same UI Table](meida/ui-table-responsiveness-mobile.png)

рис.3. Mobile View тієї самої UI Table
...
рис.4. Знімок екрана з параметрами конфігурації, доступними для типів колонок у Node-RED.
...
рис.5. Приклад ui-table, що відображає різні доступні типи комірок.
...
## Конфігурування таблиці

### Пошук і фільтрація  

Вузол `ui-table` можна налаштувати для включення панелі пошуку над таблицею. Це дозволить користувачам шукати та фільтрувати всі стовпці, а також автоматично здійснювати пошук у всіх стовпцях під час введення. Наприклад

![Example of a Data Table with Search & Filter](meida/ui-table-search.png)

рис.6. Приклад таблиці даних з увімкненими функціями пошуку та фільтрації.

### Взаємодія

Нижче наведені приклади, які показують, як виглядатиме таблиця з вибраними різними типами вибору. 

![Example of a Data Table](meida/ui-table1.png)

рис.7.  Example of a rendered data table in a Dashboard за замовченням. 

![Example of a table that has "Multi Selection" enabled](meida/ui-table-multi.png)

рис.8. Приклад таблиці з увімкненим режимом «Multi Selection».

![Example of a data table that enables a row to be selected/clicked](meida/ui-table-click.png)

рис.9. Приклад таблиці даних, у якій можна вибирати або натискати окремі рядки.

![Example of a Paginated Table](meida/ui-table-pagination.png)

рис.10. Приклад таблиці з пагінацією, яка містить 10 рядків даних, але з параметром «Max Rows», встановленим у 5.
...
рис.11. Приклад спеціального заголовка та вмісту комірки з використанням шаблону інтерфейсу користувача та таблиці даних Vuetify
